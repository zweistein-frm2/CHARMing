...
void entangle_setup(boost::filesystem::path& devicedir, boost::filesystem::path& resdir, std::vector<std::string>& prerequisites, int argc, char* argv[]) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (auto& entry : boost::make_iterator_range(boost::filesystem::directory_iterator(p), {})) {
                ...
                if (boost::algorithm::istarts_with(f, possibleName[i])) {
                    ...
                    boost::filesystem::path pdevice = entangle_root;
                    pdevice /= "device";
